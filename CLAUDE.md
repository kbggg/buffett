...
[Phase 5] 모의투자 + 소액 실전 (병행 가능)
   - KIS 모의투자 API로 시스템 시그널 검증
   - 동시에 소액 실전 (시작 100만원, 점진적 증액)
   - 매주 결과 누적 분석

[Phase 6] AWS/Vercel 배포 (이미 Supabase 쓰니 부분 완료)

[Phase 7] 본격 자본 투입 (조건부)
   - Phase 5에서 KOSPI 의미있게 초과 시 증액
   - 한 번에 큰 금액 X — 항상 점진적
```
...
3. **점진적 자본 투입 — 한 번에 큰 금액 투입 금지.** 시작 100만원 → 검증되면 단계적 증액. 모의투자 병행 권장.
